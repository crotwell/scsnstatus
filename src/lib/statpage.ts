...
import type { DataSOHType } from './jsonl_loader.ts';
...
export function doPlot<Type extends DataSOHType>(
	selector: string,
	allStats: Array<Type>,
	...
	let filtered = allStats.filter(
		(stat: Type) => selectedStations.findIndex((s) => s === stat.station) !== -1
	);
...
export function doText<Type extends DataSOHType>(
	selector: string,
	allStats: Array<Type>,
	keyFn: ((d: Type) => string) | ((d: Type) => number),
	selectedStations: Array<string>
...
export function createKeyCheckboxes(
	selector: string,
...
export function initTimeChooser(
	duration: Duration,
	callbackFn: (interval: Interval) => void
): sp.datechooser.TimeRangeChooser {
	const timeChooser = document.querySelector<sp.datechooser.TimeRangeChooser>(
		sp.datechooser.TIMERANGE_ELEMENT
	);
	if (timeChooser) {
