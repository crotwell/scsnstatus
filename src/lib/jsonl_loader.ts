...
export function loadCellStats(
	stationList: Array<string>,
	interval: Interval
): Promise<Array<CellSOH>> {
	const chan = 'CEL';
	return loadStats(stationList, chan, interval).then((jsonTextList) => {
		const allStats = jsonTextList
...
export function loadKilovaultStats(
	stationList: Array<string>,
	interval: Interval
): Promise<Array<KilovaultSOC>> {
	const chan = 'KVSOC';
	return loadStats(stationList, chan, interval).then((jsonTextList) => {
		const allStats = jsonTextList
			.filter((line) => line.length > 2)
			.map((line) => {
				...
				if ('station' in statJson === false) {
					throw new Error('No station in json object');
				}
				if ('time' in statJson === false) {
					throw new Error('No time in json object');
				}
...
export function loadComputerStats(
	stationList: Array<string>,
	interval: Interval
): Promise<Array<ComputerStat>> {
	const chan = 'DISK';
	return loadStats(stationList, chan, interval).then((jsonTextList) => {
		const allStats = jsonTextList
			.filter((line) => line.length > 2)
...
export const mib_floats = ['sinr', 'powerIn', 'byterate'];
...
export const mib_strings = ['networkServiceType', 'cellid', 'cellBand', 'networkState'];

export function json_fix_types(json: any): CellSOH {
	if ('station' in json === false) {
		throw new Error('No station in json object');
	}
	if ('time' in json === false) {
		throw new Error('No time in json object');
	}
...
export function preprocess_stats(allStats: Array<CellSOH>) {
	...
	for (const stat of sortStats) {
		...
		if (prev) {
			stat.byterate =
				(stat.cellularBytesSent - prev.cellularBytesSent) /
