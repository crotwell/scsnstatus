...
import { ringserverweb, util } from 'seisplotjs';
...
export const cloud_host = 'li1043-95.members.linode.com';
...
export class DataLatencyService {
	...
	queryLatency(fetcher?: (url: URL | string, fetchInit: RequestInit) => Promise<Response>) {
		...
		const mythis = this;
		return Promise.all([irisStats, eeyoreStats, cloudStats]).then(
			(statArray) => {
				...
				while (
					mythis.latencyHistory.length > 0 &&
					accessTime.diff(mythis.latencyHistory[0].accessTime) >
						(1 + mythis.historyLength) * mythis.updateInterval.toMillis()
				) {
		...
		if (prevLC && prevLC.latestData) {
			...
			for (let pvalue of prevLC.latestData) {
				...
				if (out.has(pkey)) {
					...
					// didn't find, reuse
					const clonePValue = new ringserverweb.StreamStat(
						pvalue.key,
						pvalue.startRaw,
						pvalue.endRaw
					);
	...
	createStreamStats(host, port, pattern) {
		...
		return conn.pullStreams(pattern).then(
			(streamStats) => {
					...
					if (stat.key === prevStat.key) {
						for (let host of HOST_LIST) {
							if (stat[host] && prevStat[host]) {
								stat.velocity[host] =
									stat[host].end.diff(prevStat[host].end) /
