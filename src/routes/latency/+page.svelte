...
<div class="latency">
	<table>
		...
		{#each data.latency.latestData as ld}
			<tr>
				<td>{ld['key']}</td>
				<td class={latencySeriousness(ld.cloud.end)}>{ld.cloud.end.toISOTime()}</td><td
					>{latencyAsText(ld.cloud.end)}</td
				><td>{ld.velocity.cloud.toFixed(2)}</td><td>{latencyVelocityIcon(ld.velocity.cloud)}</td>
				<td class={latencySeriousness(ld.eeyore.end)}>{ld.eeyore.end.toISOTime()}</td><td
					>{latencyAsText(ld.eeyore.end)}</td
				><td>{ld.velocity.eeyore.toFixed(2)}</td><td>{latencyVelocityIcon(ld.velocity.eeyore)}</td>
				<td class={latencySeriousness(ld.iris.end)}>{ld.iris.end.toISOTime()}</td><td
					>{latencyAsText(ld.iris.end)}</td
				><td>{ld.velocity.iris.toFixed(2)}</td><td>{latencyVelocityIcon(ld.velocity.iris)}</td>
			</tr>
		{/each}
	</table>
	<h5>
		...
		<span bind:this={accessTimer}></span>
	</h5>
	<h5>
		Rate averaged over last {data?.previousLatencyCache?.accessTime.toISO()}
		{#if data?.previousLatencyCache?.accessTime != null}
			{Math.round(
				Interval.fromDateTimes(data.previousLatencyCache.accessTime, data.latency.accessTime)
					.toDuration()
					.toMillis() / 1000
			)}
		{:else}
