...
//                           ) {
export function scatterplot(selector, data, keyFn, allStations, colorForStation) {
	...
	// Add X axis
	const x = d3_scaleUtc().domain([start, end]).range([0, width]).nice();
	svg.append('g').call(d3_axisTop(x));
	svg
		.append('g')
	...
	if (numberData) {
		...
		yNumberScale = d3_scaleLinear().domain([min, max]).range([height, 0]).nice().nice();
		y = yNumberScale;
	} else {
		console.log(`ord Data`);
		yOrdinalScale = d3_scalePoint().domain(allOrdinalVals).range([height, 0]);
		y = yOrdinalScale;
	}
	svg.append('g').call(d3_axisLeft(y));

	svg.append('g').attr('transform', `translate(${width}, 0)`).call(d3_axisRight(y));

	const highlight = function ({}, d) {
		d3_selectAll('.dot').transition().duration(200).style('fill', 'lightgrey').attr('r', 3);
	...
	svg
		...
		.attr('class', function (d) {
			return 'dot ' + d.station;
		})
		.attr('cx', function (d) {
			return x(d['time'].toJSDate());
		})
		.attr('cy', function (d) {
			return y(keyFn(d));
		})
		...
		.style('fill', function (d) {
			return colorForStation.get(d.station);
		})
