...
export function doPlot(config) {
  ...
  if (
    config.station &&
    (!heliDiv || window.getComputedStyle(heliDiv) === "none")
  ) {
    document.querySelector("#heli").setAttribute("style", "display: none;");
    document
      .querySelector("#seismograph")
...
export function doPlotHeli(config) {
  ...
  if (true) {
    ...
    channelPromise = sp.util
      .doFetchWithTimeout(url, fetchInitOptions)
  ...
  return channelPromise
    .catch((e) => {
      ...
      return new Promise((resolve) =>
        setTimeout(resolve, 2000, channelQuery.queryChannels()),
      );
    })
    .then((netArray) => {
      ...
      const matchChannels = sp.stationxml.findChannels(
        netArray,
        ".*",
        config.station,
        config.locCode,
      ...
      for (let c of matchChannels) {
        if (
          c.channelCode.endsWith(config.orientationCode) ||
          (config.altOrientationCode &&
            c.channelCode.endsWith(config.altOrientationCode))
        ) {
          chanTR.push(
      ...
      if (firstChan) {
        document.querySelector("span.textNetCode").textContent =
          firstChan.networkCode;
        document.querySelector("span.textStaCode").textContent =
          firstChan.stationCode;
        document.querySelector("span.textLocCode").textContent =
          firstChan.locationCode;
        document.querySelector("span.textChanCode").textContent =
          firstChan.channelCode;
      }
      hash.heli = document.querySelector("sp-helicorder");
      if (hash.heli) {
        // draw empty SDD so clear existing and fix labels
    ...
    .then((hash) => {
      ...
      hash.chanTR
        ...
        .forEach((sdd) => {
          if (
            hash.config.dominmax &&
            sdd.networkCode === "CO" &&
            sdd.sourceId.bandCode === "H"
          ) {
      ...
      if (hash.config.dominmax && minMaxSddList.length > 0) {
        ...
        minMaxSddList = minMaxSddList
          .map((ct) => {
            let chanCode =
              "L" + hash.minMaxInstCode + ct.channel.channelCode.charAt(2);
            ...
            return sp.seismogram.SeismogramDisplayData.fromChannelAndTimeWindow(
              fake,
              ct.timeRange,
            );
          })
    ...
    .then((hash) => {
      ...
      hash.chantrList.forEach((ctr) => {
        if (
          ctr.channel.channelCode ===
            `L${hash.minMaxInstCode}${hash.config.orientationCode}` ||
          ctr.channel.channelCode ===
            `L${hash.minMaxInstCode}${hash.config.altOrientationCode}`
        ) {
          minMaxSeismogram = ctr.seismogram;
        } else if (
          ctr.channel.channelCode ===
            `${hash.bandCode}${hash.instCode}${hash.config.orientationCode}` ||
          ctr.channel.channelCode ===
            `${hash.bandCode}${hash.instCode}${hash.config.altOrientationCode}`
        ) {
          minMaxSeismogram = ctr.seismogram;
          document.querySelector("span.textChanCode").textContent =
            ctr.channel.channelCode;
          ...
          throw new Error(
            `Cannot find trace ends with L${hash.minMaxInstCode}${hash.config.orientationCode} or L${hash.minMaxInstCode}${hash.config.altOrientationCode} or ${hash.bandCode}${hash.instCode}${hash.config.orientationCode}`,
          );
        }
      });
      if (!minMaxSeismogram) {
...
export function queryEarthquakes(hash) {
  return Promise.resolve(hash)
    ...
    .then((hash) => {
      ...
      if (hash.regionalQuakes.length > 0)
        hash.quakes = hash.quakes.concat(hash.regionalQuakes);
      if (hash.globalQuakes.length > 0)
        hash.quakes = hash.quakes.concat(hash.globalQuakes);
      if (hash.seisData) {
    ...
    .then((hash) => {
      ...
      hash.quakes.forEach((quake) => {
        let ttresult = new sp.traveltime.TraveltimeQuery()
          ...
          .evlat(quake.latitude)
          .evlon(quake.longitude)
          .stalat(mystation.latitude)
          .stalon(mystation.longitude)
          ...
          .then(function (ttimes) {
            ...
            for (let p = 0; p < ttimes.arrivals.length; p++) {
              if (
                (ttimes.arrivals[p].phase.startsWith("P") ||
                  ttimes.arrivals[p].phase.startsWith("p")) &&
                (!firstP || firstP.time > ttimes.arrivals[p])
              ) {
                firstP = ttimes.arrivals[p];
              }
              if (
                (ttimes.arrivals[p].phase.startsWith("S") ||
                  ttimes.arrivals[p].phase.startsWith("s")) &&
                (!firstS || firstS.time > ttimes.arrivals[p])
              ) {
...
export function updateHeliAmpConfig(hash, heliConfig) {
  if (hash.config.amp === "max") {
    ...
    typeof hash.config.amp === "string" &&
    hash.config.amp.endsWith("%")
  ) {
    heliConfig.fixedAmplitudeScale = [0, 0];
    const percent =
      Number(hash.config.amp.substring(0, hash.config.amp.length - 1)) / 100;
    heliConfig.maxVariation =
      percent * (hash.seisData.max - hash.seisData.mean);
...
export function drawSeismograph(hash) {
  ...
  let friendChannels = Array.from(sp.stationxml.allChannels(hash.netArray));
  friendChannels = friendChannels.filter(
    (ch) => ch.station.stationCode === hash.station,
  );
  ...
  const overlapQuakes = hash.traveltimes
    .filter((tt) => {
      const lastArrival = tt.ttimes.arrivals.reduce((acc, cur) =>
        acc.time > cur.time ? acc : cur,
      );
      const quakeInterval = sp.luxon.Interval.after(tt.quake.time, {
        seconds: lastArrival.time,
      });
