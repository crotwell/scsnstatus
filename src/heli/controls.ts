...
export function handleFilteringChange(
  config,
  type,
  lowcut,
  highcut,
...
export function handleAmpChange(config, value, redrawFun) {
  if (value === "max") {
    ...
    config.amp = 10000;
    document.querySelector("#amp").querySelector("input#fixedAmpText").value =
      config.amp;
  }
...
export function setupEventHandlers(config, loadAndPlotFun, redrawFun) {
    ...
    dispElements.forEach((orgDispItem) => {
      if (orgDispItem.plottype.startsWith(sp.organizeddisplay.SEISMOGRAPH)) {
        ...
        seismograph.seisData.forEach((sdd) => {
          const dispWindow =
            seismograph.displayTimeRangeForSeisDisplayData(sdd);

          let start = sdd.start;
          let end = sdd.end;
          if (dispWindow.start < sdd.start) {
            start = dispWindow.start;
          }
          if (dispWindow.end > sdd.end) {
            end = dispWindow.end;
          }
  ...
  bandInstCodeList.forEach((bandinst) => {
    ...
    button.checked =
      bandinst.charAt(0) === config.bandCode &&
      bandinst.charAt(1) === config.instCode;
    button.addEventListener("click", (event) => {
  ...
  document
    .querySelector("button#loadToday")
    .addEventListener("click", function (d) {
      config.endTime = luxon.DateTime.utc()
        .endOf("day")
        .plus({ millisecond: 1 })
        .toISO();
      updateDateChooser(config);
...
export function updatePageForConfig(currentConfig) {
  ...
  if (currentConfig && currentConfig.filter) {
    ...
    document.querySelector("input#lowcut").textContent =
      currentConfig.filter.lowcut;
    document.querySelector("input#highcut").value =
      currentConfig.filter.highcut;
    document.querySelector("input#highcut").textContent =
      currentConfig.filter.highcut;
  }
  ...
  if (currentConfig) {
    if (
      typeof currentConfig.amp === "string" &&
      currentConfig.amp.endsWith("%")
    ) {
      let percent = Number(
        currentConfig.amp.substring(0, currentConfig.amp.length - 1),
      );
      ...
      document.querySelector("input#fixedAmpText").textContent =
        currentConfig.amp;
    } else {
      // default to max?
      document.querySelector("#maxAmp").checked = true;
...
function updateEarthquakeQueryParam(currentConfig, id, defaultValue) {
  ...
  document.querySelector("div#" + region).querySelector("input#" + id).value =
    currentConfig[id];
}
...
export function enableFiltering(heliDataIsMinMax) {
  ...
  document.querySelector("#filtering").querySelector("input#allpass").disabled =
    heliDataIsMinMax;
  document.querySelector("#filtering").querySelector("input#lowpass").disabled =
    heliDataIsMinMax;
  document
